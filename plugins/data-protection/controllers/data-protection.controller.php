        ...
        try {
            $st = $this->link->prepare(
                "INSERT INTO dp_datasets
                 (table_dataset, label_dataset, pk_dataset, subject_keys_dataset, fields_dataset, sensitive_dataset, anonymize_dataset, purpose_dataset, legal_basis_dataset, retention_days_dataset, date_created_dataset)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
                 ON DUPLICATE KEY UPDATE
                   ...
                   retention_days_dataset=VALUES(retention_days_dataset)"
            );
            $st->execute([
                ...
                trim((string) ($p['purpose'] ?? '')), trim((string) ($p['legal_basis'] ?? '')), $retention,
            ]);
            return ['success' => true];
        } catch (Exception $e) {
